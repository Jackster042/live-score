...
describe('Matches API', () => {
  ...
  describe('GET /matches', () => {
    ...
    it('should return list of matches', async () => {
      ...
      await db.insert(matches).values([
        {
          ...matchData1,
          startTime: new Date(matchData1.startTime),
          endTime: new Date(matchData1.endTime),
        },
        {
          ...matchData2,
          startTime: new Date(matchData2.startTime),
          endTime: new Date(matchData2.endTime),
        },
      ]);
    ...
    it('should respect limit parameter', async () => {
      ...
      const response = await request(app).get('/matches?limit=3').expect(200);

      expect(response.body.data).toHaveLength(3);
    });

    it('should reject invalid limit', async () => {
      const response = await request(app).get('/matches?limit=invalid').expect(400);
  ...
  describe('POST /matches', () => {
    it('should create a new match', async () => {
      ...
      const response = await request(app).post('/matches').send(matchData).expect(201);
