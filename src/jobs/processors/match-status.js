 ...
 * @param {number} job.data.matchId - Match ID
 * @param {string} job.data.fromStatus - Expected current status
 * @param {string} job.data.toStatus - Target status
 */
export async function processMatchStatusTransition(job) {
  ...
  if (match.status !== fromStatus) {
    console.warn(`   ⚠️ Match ${matchId} status is '${match.status}', expected '${fromStatus}'. Skipping transition.`);
 ...
 * Job processor map
 */
export const matchStatusProcessors = {
