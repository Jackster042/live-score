 ...
 * @param {Object} processors - Map of job name → processor function
 * @param {Object} opts - Worker options
 * @returns {Worker}
 */
function createWorker(queueName, processors, opts = {}) {
  ...
  const worker = new Worker(
    ...
    async (job) => {
      ...
      try {
        ...
        const duration = Date.now() - startTime;
        console.error(`   ❌ Job failed: ${job.name} (${duration}ms)`, err.message);
        throw err;
      }
    },
    {
...
export function initWorkers() {
  ...
  const matchStatusWorker = createWorker(
    queueKeys.matchStatus(),
    matchStatusProcessors,
    { 
